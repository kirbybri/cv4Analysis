...
from sliceAnalyzeSample import SLICE_ANALYZE_SAMPLE
...
class CV4_ANALYZE_WAVEFORM(object):
  ...
  def __init__(self, fileName = None):
    ...
    self.analyzeSample = SLICE_ANALYZE_SAMPLE()
    ...
    #self.cv3tbAnalyzeSample.mdacWeights = [0,0,0,0,0,0,0,0]
    #self.cv3tbAnalyzeSample.sarWeights = [0,0,0,0,16384*2,16384,8192,4096,2048,1024,512,256,128,64,32,16,8,4,2,1]
  ...
  def plotValsFft(self,measNum="",vals=[],psd_x=[],psd=[],label=""):
    #return
  ...
  def getMeasChData(self,measNum=None,chId=None,gain=None):
    if chId == None or self.runResultsDict == None or measNum == None or gain == None:
      return None
    if "results" not in self.runResultsDict:
    ...
    if gain not in chanData :
      print("ERROR, MISSING GAIN",gain)
      return None
  ...
  def viewWaveform(self,chId=None,measNum=None,doPrint=False,doPlot=False):
    ...
    #return
    if doPlot:
      self.plotVals(measNum,vals[0:200],psd_x,psd)
    #return np.mean(vals)
    return np.mean(vals), np.std(vals), np.max(vals),np.min(vals)
