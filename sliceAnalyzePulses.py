...
class SLICE_ANALYZE_PULSES(object):
  ...
  def findPulses(self,chWf):
    ...
    #print(maxSamp,meanSamp,threshold)
    ...
    for pulseSet in pulseSetDict :
      ...
      for pulseNum, pulseTimeInfo in enumerate(pulseSetInfo) :
        ...
        fitPulseSetDict[setCount].append( {"idx":idx,"pulseTime":pulseTime,"fitPulseTime":slope*pulseNum + intercept } )
  ...
  def getPulses(self,chWf,fitPulseSetDict,chWfBin=[]):
    ...
    if len(pedVals) < 3:
      return None
    pedVal = np.mean(pedVals)
    pedRms = np.std(pedVals)
    pedErr = pedRms/np.sqrt(len(pedVals))
    #print("PED VAL",pedVal,pedRms,pedErr)

    #get avg pulse
    bins = np.linspace(-10,25,1401)
    ...
    for binNum in binsDict :
      ...
      for sampNum, samp in enumerate(pulse_x):
        if pulse_x[sampNum] >= low and pulse_x[sampNum] < high :
          binsDict[binNum]["samples"].append( pulse_y[sampNum] )
    ...
    if sampTime_rise5Per != None and sampTime_rise100Per != None :
      print("RISETIME ", sampTime_rise100Per - sampTime_rise5Per)
    ...
    if False :
      ...
      plot_x = [x*25 for x in pulse_x]
      plot_y = [x for x in pulse_y]
      plot_y_bin = [x for x in pulse_y_bin]
    ...
    return fitPulseSetDict, binsDict, pedVal, pedRms, avgPulseHeight    
